...
public class RegisterAccountSteps extends BaseSetup{
	...
	public void clickOnSignUp() throws InterruptedException {
		...
		wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(SelectorsResources.SIGN_IN_LINK_ON_CREATE)));
		Thread.sleep(2000);
	}
	
	@Test(priority = 7, description = "Try to create account o empty fields")
	//this method will be used on each step to check if the create button become active
	...
	public static void enterInvalidPassword () throws InterruptedException {
		...
		createButton.click();
		Thread.sleep(2000);
		Assert.assertTrue(driver.findElement(By.cssSelector(SelectorsResources.INCORRECT_PASSWORD_MESSAGE)).isDisplayed());
	}
	...
	public static void clickOnCreateNewAccountButton () {
		WebElement createButton = driver.findElement(By.cssSelector(SelectorsResources.CREATE_NEW_ACCOUNT));
		createButton.click();
//		WebElement goToYourBoards = driver.findElement(By.cssSelector("a[href*='/']"));
	//	wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("a[href*='/']")));
		...
		try {
			//dodaj pomin to
			wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(SelectorsResources.BOARD_NAME_INPUT)));
			driver.findElement(By.cssSelector(SelectorsResources.BOARD_NAME_INPUT)).sendKeys("Test_Board"+number);
			Thread.sleep(200);
			driver.findElement(By.cssSelector(SelectorsResources.CONTINUE_ARROW)).click();
